...
**M5 まで実装済み。** MVP（推論 + 履歴、論点30）に加えて、アノテーション・データセット・転移学習・評価まで動く。
...
転移学習は**バックボーンを凍結し、YOLOv8 相当の検出ヘッド**（3スケール・DFL・タスク整合割当）
だけを学習する。**学習は WebGL で行う**（WebGPU では割当が壊れる。推論は WebGPU）。
