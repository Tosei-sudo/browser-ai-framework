...
| 03 ストレージ設計 | 25〜29 | 未決 | [04-storage-design.md](./04-storage-design.md) |
...
| # | 論点 | 優先度 | 状態 |
| --- | --- | --- | --- |
| 25 | 実体（画像・重み）の格納先 | 高 | 未決 |
| 26 | 永続化要求（`persist()`）の扱い | 高 | 未決 |
| 27 | IndexedDB / OPFS 間の不整合の回復方式 | 中 | 未決 |
| 28 | スキーマ変更（マイグレーション）の方針 | 中 | 未決 |
| 29 | 参照カウントの管理方式 | 低 | 未決 |
