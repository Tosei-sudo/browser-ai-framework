...
これは設計思想③と真っ向から衝突する。対処の手段は Storage API の永続化要求（`navigator.storage.persist()`）で、許可されるとブラウザによる自動削除の対象から外れる。ただし許可の条件はブラウザごとに異なり、拒否されることもある。

**方針: 起動時に `persist()` を要求し、拒否された場合は警告を出すが操作はブロックしない**（論点26）。

- 起動時に一度要求する。拒否された状態では警告バナーを常時表示し、エクスポート（方針17）を促す。
- 学習を開始しようとしたときに再度警告する。ただし操作自体は許可する。
- ブラウザによっては訪問を重ねることで後から許可されるため、一度の拒否で諦めない。

エクスポート機能（方針17）は、永続化が得られない環境における唯一の保険として位置づけられる。実装の優先度をそれに応じて上げる。

**この懸念は実測で再現している。** Chrome + localhost の環境で `persist()` が拒否された（[06 §3.4](./06-poc-results.md#34-重要な発見--persist-が拒否された)）。拒否は「まれな異常系」ではなく初期状態として起こりうるものとして扱う。
...
**メタデータは IndexedDB、実体は OPFS に置く**（論点25）。OPFS が利用できない環境では、`BlobStore` の IndexedDB 実装にフォールバックする。`BlobStore` を Port として切ってあるため（[03 §5](./03-system-architecture.md#port抽象インターフェース)）、この差し替えは他の層に影響しない。
...
**削除時に都度集計する**（論点29）。`ImageBlob` に `ref_count` は持たせない。

削除は起動時やユーザー操作時にまとめて行う処理であり、リアルタイム性を要求されない。参照カウントを保持する方式は判定が O(1) になるが、更新漏れによるずれは「実体が消せない」または「参照されているのに消える」として表面化するまで気づけず、リスクが利点を上回る。
...
- 属性の追加は既定値で吸収し、移行関数を書かずに済ませる。
- ストア・インデックスの追加は移行関数で行う。エンティティ構成は確定しているため大きな構造変更は起きにくいが、**インデックスの追加は運用中に必ず発生し、後方互換では吸収できない**。
- 移行関数はバージョンごとに積み上げ、任意の古いバージョンから順に適用する。

### 8.3 移行に失敗した場合

**旧スキーマのまま読み取り専用で開き、エクスポートを促す**（論点28の付随決定）。

ローカルにしかデータがないため、初期化は完全な喪失を意味する。移行に失敗しても、少なくともデータを取り出せる状態は維持する。読み取り専用モードのUI（新規作成・学習・編集を禁止し、閲覧とエクスポートのみ許可）が必要になる。
...
決定ログの通し番号を継続している。選択肢と判断理由は [02-open-decisions.md](./02-open-decisions.md) を参照。

| # | 論点 | 決定 |
| --- | --- | --- |
| 25 | 実体の格納先 | メタは IndexedDB、実体は OPFS。非対応環境は `BlobStore` を IndexedDB 実装にフォールバック |
| 26 | 永続化要求の扱い | 起動時に `persist()` を要求。拒否時は警告とエクスポート促進、操作はブロックしない |
| 27 | 不整合の回復方式 | 実体を先に書き、メタを後からコミット。孤児ファイルは起動時にGC |
| 28 | マイグレーション方針 | 移行関数と後方互換を併用。失敗時は旧スキーマで読み取り専用 |
| 29 | 参照カウントの管理 | 削除時に都度集計。`ref_count` は持たない |

### 実装フェーズへ持ち越す事項

| 事項 | 関連 |
| --- | --- |
| OPFS のブラウザ対応状況の確認と、フォールバック実装の要否判断 | 論点25 |
| 読み取り専用モードのUI仕様（閲覧とエクスポートのみ許可する範囲） | 論点28 |
| 推論履歴の上限件数（容量ではなく一覧の見やすさで決めてよい — §9） | 方針14 |
